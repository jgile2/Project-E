...
public class TileCondenser extends EmcContainerTile implements ISidedInventory {
    ...
    public boolean canInsertItem(int i, ItemStack itemstack, int j) {
        ...
        if (i > items.length - 2) return false;
    ...
    public void updateEntity() {
        ...
        if (worldObj != null && !worldObj.isRemote && playersCurrentlyUsingChest != 0 && (ticksSinceSync + xCoord + yCoord + zCoord) % 200 == 0) {
            ...
            List<EntityPlayer> var2 = worldObj.getEntitiesWithinAABB(
                    EntityPlayer.class,
                    AxisAlignedBB.getAABBPool().getAABB(xCoord - var1, yCoord - var1, zCoord - var1, xCoord + 1 + var1, yCoord + 1 + var1,
                            zCoord + 1 + var1));
        ...
        for (int i = 0; i < this.items.length - 3; i++) {
            ...
            if (is != null && is.stackSize > 0) {
                if (!resultItem.isItemEqual(is)) {
                    ...
                    if (is.stackSize < is.getMaxStackSize()) if (resultSlot < 0) resultSlot = i;
                }
            } else {
                if (resultSlot < 0) resultSlot = i;
            }
        }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        if (resource.getFluid() != PEFluids.liquidEMC) return 0;
        
        ItemStack item = getStackInSlot(items.length - 3);
        if (item != null && item.getItem() instanceof IEmcContainerItem) {
            int amt = Math.min(Math.min(resource.amount, maxEmcInput), ((IEmcContainerItem) item.getItem()).getMaxStoredEmc(item)
                    - ((IEmcContainerItem) item.getItem()).getStoredEmc(item));
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        ...
        if (resource.getFluid() != PEFluids.liquidEMC) return null;
    ...
    public int getEmcStored() {
        ...
        ItemStack item = getStackInSlot(items.length - 3);
        if (item != null && item.getItem() instanceof IEmcContainerItem) return ((IEmcContainerItem) item.getItem()).getStoredEmc(item);
    ...
    public int getMaxEmcStored() {
        ...
        ItemStack item = getStackInSlot(items.length - 3);
        if (item != null && item.getItem() instanceof IEmcContainerItem) return ((IEmcContainerItem) item.getItem()).getMaxStoredEmc(item);
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        ...
        ItemStack item = getStackInSlot(items.length - 3);
        if (item != null && item.getItem() instanceof IEmcContainerItem) return new FluidTankInfo[] { new FluidTankInfo(new FluidStack(
                PEFluids.liquidEMC, ((IEmcContainerItem) item.getItem()).getStoredEmc(item)), Math.max(
                ((IEmcContainerItem) item.getItem()).getMaxStoredEmc(item), 1)) };

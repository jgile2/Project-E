...
import jgile2.mods.projecte.lib.FluidHelper;
import jgile2.mods.projecte.lib.ItemHelper;
...
public class WrappedStack implements Comparable<WrappedStack>, JsonDeserializer<WrappedStack>, JsonSerializer<WrappedStack>
{
    ...
    public int hashCode()
    {
        ...
        {
           // hashCode = (37 * hashCode) + ((ItemStack) wrappedStack)
    ...
    public String toString()
    {
        ...
        if (wrappedStack instanceof ItemStack)
        {
            ...
            try
            {
                stringBuilder.append(String.format("%sxitemStack[%s:%s:%s:%s]", stackSize, itemStack, itemStack.getItemDamage(), itemStack.getUnlocalizedName(), itemStack.getItem().getClass().getCanonicalName()));
            }
            ...
            if (jsonWrappedStack.get("wrappedStack") != null && !jsonWrappedStack.get("wrappedStack").isJsonPrimitive())
            {
                if (className != null)
                {
                    if (className.equalsIgnoreCase(ItemStack.class.getSimpleName()))
                    {
                        ...
                        if (stackSize > 0)
                        {
                           // itemStack = new ItemStack(jsonItemStack, stackSize, jsonItemStack.itemDamage);

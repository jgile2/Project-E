...
public class ContainerEnergyCollectorMK2 extends Container {

	private TileEnergyCollectorMK2 tile;

	public ContainerEnergyCollectorMK2(InventoryPlayer inventory, TileEnergyCollectorMK2 tileentity) {
		this.tile = tileentity;

		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 3; j++) {
				this.addSlotToContainer(new SlotEmcFuel(tileentity, j + i * 3, 18 + j * 18, 8 + i * 18));
			}
		}
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 9; j++) {
				this.addSlotToContainer(new Slot(inventory, j + i * 9 + 9, 20 + j * 18, 84 + i * 18));
			}
		}

		for (int i = 0; i < 9; i++) {
			this.addSlotToContainer(new Slot(inventory, i, 20 + i * 18, 142));
		}
	}
